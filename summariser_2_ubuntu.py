...
class SummaryTool(object):
    ...
    def get_best_sentence(self, paragraph, sentences_dic):
        ...
        best_sentence, secondbest = "", ""
        max_value, secondmax = 0, 0
        for s in sentences:
            strip_s = self.format_sentence(s)
            if strip_s:
                if sentences_dic[strip_s] > max_value: # and len(s)>10:
                    secondmax = max_value
    ...
    with open(filename) as chat:
        for line in chat:
            ...
            if m:
                ...
                if sender:
                    senderlen = len(sender.group(0)[:])
                if message:
                    item = ['1/1/1980', message.group(0)[senderlen:]]
                    chatlog.append(item)
